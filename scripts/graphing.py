...
def generate_skill_score_bar_charts(
    ...
    for focus_area, dict_skills in dict_scores.items():
        ...
        path_focus_area = pathlib.Path(__file__).parent.parent / "tmp" / filename_ending
...
def generate_focus_area_spider_plot(
    ...
    path_spiderplot_graph = (
        pathlib.Path(__file__).parent.parent / "tmp" / "focus_area_spider_plot.jpg"
    )
...
def generate_skill_score_colorbar_plots(
    ...
    for skill_dict in dict_scores.values():
        for skill, score in skill_dict.items():
            ...
            file_name = skill + ".jpg"
            path_skill_gauge_chart = pathlib.Path(__file__).parent.parent / "tmp" / file_name
...
def generate_color_bar_plot(
    ...
    path_color_bar = pathlib.Path(__file__).parent.parent / "tmp" / file_name
...
def generate_line_chart(
    ...
    segments = len(pace_time_series_x) - 1
    points_per_segment = 100
    pace_time_series_x_scatter = np.linspace(
        min(pace_time_series_x), max(pace_time_series_x), segments * points_per_segment
    )
    ...
    ax[0].set_xlim(0, max(pace_time_series_x_scatter))
    ...
    ax[0].set_xlim(min(pace_time_series_x_scatter), max(pace_time_series_x_scatter))
    ...
    path_line_chart = pathlib.Path(__file__).parent.parent / "tmp" / file_name
...
def generate_stacked_bar_chart_pauses(dict_pauses: Dict) -> None:
    ...
    path_stack_bar_chart = (
        pathlib.Path(__file__).parent.parent / "tmp" / "pauses_stacked_bar_chart.jpg"
    )
